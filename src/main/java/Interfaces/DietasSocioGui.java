...
public class DietasSocioGui extends javax.swing.JDialog {
    ...
    public DietasSocioGui(final java.awt.Frame parent, boolean modal, final int idSocio) {
        super(parent, modal);
        initComponents();
        estaVentana = this;
                    ...
                    if (row > -1) {
                        ...
                        switch (d.getReturnStatus()) {
                            case VerDietaEditarGui.RET_VER:
                                LazyList<AlimentosDietas> alimentosDietas = AlimentosDietas.where("dieta_id = ?", (Integer) tblDietas.getValueAt(tblDietas.getSelectedRow(), 2));
                                VerDietaGui verDietaGui = new VerDietaGui(parent, true, alimentosDietas, (String) tblDietas.getValueAt(tblDietas.getSelectedRow(), 0), (String) tblDietas.getValueAt(tblDietas.getSelectedRow(), 1), -1, -1);
                            ...
                            case VerDietaEditarGui.RET_AGREGAR:
                                ...
                                if (agregarDieta.getReturnStatus() == AgregarDietaSocioGui.RET_OK) {
                                    ...
                                    LazyList<SociosDietas> sociosDietas  =SociosDietas.where("socio_id = ?", idSocio);
                                    cargarEnTablaDietaSocio(sociosDietas);
        ...
        tblDietasSocio.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                if (evt.getClickCount() == 2) {
                    LazyList<AlimentosDietas> alimentosDietas = AlimentosDietas.where("dieta_id = ?", (Integer) tblDietasSocio.getValueAt(tblDietasSocio.getSelectedRow(), 5));
                    VerDietaGui verDietaGui = new VerDietaGui(parent, false, alimentosDietas, (String) tblDietasSocio.getValueAt(tblDietasSocio.getSelectedRow(), 0), (String) tblDietasSocio.getValueAt(tblDietasSocio.getSelectedRow(), 1), (Integer) tblDietasSocio.getValueAt(tblDietasSocio.getSelectedRow(), 5), idSocio);
                    verDietaGui.setLocationRelativeTo(estaVentana);
                    verDietaGui.setVisible(true);
                }
            }
    ...
    private void busquedaDietas() {
        Base.openTransaction();
        LazyList<Dieta> dietas=Dieta.where("descripcion like ? or nombre like ?  ", "%" + txtBusquedaDietas.getText() + "%", "%" + txtBusquedaDietas.getText() + "%");
        cargarEnTablaDieta(dietas);
        Base.commitTransaction();
    }
